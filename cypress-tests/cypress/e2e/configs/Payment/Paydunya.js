...
// the merchant's business profile name (`profile_name`) as `store.name`. The
// business profile used for these tests must therefore have a non-empty name.
// If the profile name cannot be resolved, the order-create leg fails with
// `InvalidConnectorConfig` (`merchant profile name`) and every wallet flow
// below errors instead of returning the asserted `processing` response.
...
// from `(payment_method_type, billing.country)` (see `PaydunyaOperator`). Card
